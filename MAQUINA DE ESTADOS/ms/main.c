...
// Estructura para manejar las señales de entrada/salida//SENSORES
struct DATA_IO
{
    unsigned int LSC:1;      // Sensor de posición cerrado
    unsigned int LSA:1;      // Sensor de posición abierto
    unsigned int SPP:1;      // Señal para iniciar el proceso de apertura
    ...
    unsigned int DATOS_READY:1; // Señal para indicar si los datos están listos
...
int Func_CERRANDO(void)
{
    ...
    for (;;)
    {
        if (data_io.LSC == TRUE) // Si el sensor de cerrado está activo//cerro
        {
            return ESTADO_CERRADO;
        }
        if (data_io.Cont_RT > RT_MAX) // Si se supera el tiempo máximo hay un error y no cambio a cerrado
        {
            return ESTADO_ERROR;
        }
    }
...
int Func_ABRIENDO(void)
{
    ...
    for (;;)
    {
        ...
        if (data_io.Cont_RT > RT_MAX) // Si se supera el tiempo máximo hay un error y no cambio a abierto
        {
            return ESTADO_ERROR;
        }
    }
...
void delay(void)
{
    // Implementación del delay específica del microcontrolador
}
